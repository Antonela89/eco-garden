import { getMyGarden, deletePlantFromGarden, updatePlantStatusInGarden } from './api.js';
...
export const initDashboard = async (user) => {
    ...
    gardenContainer.addEventListener('click', async (e) => {
        ...
        const statusButton = e.target.closest('[data-action="toggle-status-menu"]');
        ...
        if (deleteButton) {
            ...
            openModal(createConfirmModalContent(`¿Realmente quieres quitar "${plantName}" de tu huerta?`, 'Sí, quitar'), 'md');

            document.getElementById('confirm-action-button').addEventListener('click', async () => {
                try {
                    await deletePlantFromGarden(plantId);
                    closeModal();
                    loadGarden(); // Recargar la huerta para ver los cambios
                } catch (error) {
